...
local MouseCarryKey = string.byte("M")
...
function OrbwalkingOnLoad()
	AutoCarry.Orbwalker = TargetSelector(TARGET_LESS_CAST_PRIORITY, getTrueRange(), DAMAGE_PHYSICAL, false)
...
function LastHitOnDraw()
	if DisplayMenu.minion and enemyMinions.objects[1] and ValidTarget(enemyMinions.objects[1]) and not isMelee then
		DrawCircle(enemyMinions.objects[1].x, enemyMinions.objects[1].y, enemyMinions.objects[1].z, 90, 0x19A712)
	end
end
...
local items =
	{
		...
		{name = "Blackfire Torch", menu = "BFT", id=3188, range = 750, reqTarget = true, slot = nil },
...
function OnLoad()
        ...
        elseif myHero.charName == "Akali" then
                aaSpellName = "akalibasicattack"
                startAttackSpeed = "0.694"
        ...
        elseif myHero.charName == "Kassadin" then
                aaSpellName = "kassadinbasicattack"
                startAttackSpeed = "0.640"

        elseif myHero.charName == "Katarina" then
                aaSpellName = "katarinabasicattack"
                startAttackSpeed = "0.658"
        ...
        elseif myHero.charName == "Lee Sin" then
                aaSpellName = "leesinbasicattack"
                startAttackSpeed = "0.651"

        elseif myHero.charName == "Lucian" then
                aaSpellName = "lucianbasicattack"
                startAttackSpeed = "0.638"

				elseif myHero.charName == "Malphite" then
                aaSpellName = "malphiteebasicattack"
                startAttackSpeed = "0.638"

        elseif myHero.charName == "Maokai" then
                aaSpellName = "maokaibasicattack"
                startAttackSpeed = "0.694"

        elseif myHero.charName == "Master Yi" then
                aaSpellName = "masteryibasicattack"
                startAttackSpeed = "0.679"

        elseif myHero.charName == "Miss Fortune" then
                aaSpellName = "missfortunebasicattack"
                startAttackSpeed = "0.656"

        elseif myHero.charName == "Mordekaiser" then
                aaSpellName = "mordekaiserbasicattack"
                startAttackSpeed = "0.694"
 
         elseif myHero.charName == "Nami" then
                aaSpellName = "namibasicattack"
                startAttackSpeed = "0.644"
 
         elseif myHero.charName == "Nasus" then
                aaSpellName = "nasusbasicattack"
                startAttackSpeed = "0.638"
 
         elseif myHero.charName == "Nautilus" then
                aaSpellName = "nautilusbasicattack"
                startAttackSpeed = "0.613"
        ...
        elseif myHero.charName == "Nidalee" then
         ...
         elseif myHero.charName == "Nocturne" then
                aaSpellName = "nocturnebasicattack"
                startAttackSpeed = "0.668"
 
         elseif myHero.charName == "Olaf" then
                aaSpellName = "olafbasicattack"
                startAttackSpeed = "0.694"
       ...
       elseif myHero.charName == "Talon" then
        ...
        elseif myHero.charName == "Trundle" then
                aaSpellName = "trundlebasicattack"
                startAttackSpeed = "0.670"
 
        elseif myHero.charName == "Tryndamere" then
                aaSpellName = "tryndamerebasicattack"
                startAttackSpeed = "0.670"
       ...
       elseif myHero.charName == "Udyr" then
                aaSpellName = "udyrbasicattack"
                startAttackSpeed = "0.658"

        elseif myHero.charName == "Urgot" then
                aaSpellName = "urgotbasicattack"
                startAttackSpeed = "0.644"
       ...
       elseif myHero.charName == "Volibear" then
                aaSpellName = "volibearbasicattack"
                startAttackSpeed = "0.658"
 
        elseif myHero.charName == "Warwick" then
                aaSpellName = "warwickbasicattack"
                startAttackSpeed = "0.679"

       elseif myHero.charName == "MonkeyKing" then
                aaSpellName = "monkeykingbasicattack"
                startAttackSpeed = "0.658"

       elseif myHero.charName == "Xin Zhao" then
                aaSpellName = "xinzhaobasicattack"
                startAttackSpeed = "0.672"

       elseif myHero.charName == "Yasuo" then
                aaSpellName = "yasuobasicattack"
                startAttackSpeed = "0.658"

        elseif myHero.charName == "Zac" then
                aaSpellName = "zacbasicattack"
                startAttackSpeed = "0.638"

       elseif myHero.charName == "Zed" then
                aaSpellName = "zedbasicattack"
                startAttackSpeed = "0.658"
...
function OnDraw()
	...
	if FarmMenu.MinionCircle and killableMinion then
		DrawCircle(killableMinion.x, killableMinion.y, killableMinion.z, 110, 0xFF0000)
	end
...
function getSpellList()
		...
		elseif myHero.charName == "Renekton" then
			spellArray = {
			{ spellKey = _W, range = 300, speed = 2, delay = 0, width = 200, configName = "ruthlesspredator", displayName = "W (Ruthless Predator)", enabled = true, skillShot = false, minions = true, reset = true, reqTarget = false },
			}			
		elseif myHero.charName == "Jax" then
			spellArray = {
     ...
     elseif myHero.charName == "Lucian" then
			spellArray = {
			{ spellKey = _Q, range = 570*2, speed = math.huge, delay = 350, width = 65, configName = "PiercingLight", displayName = "Q (Piercing Light)", enabled = true, skillShot = false, minions = false, reset = false, reqTarget = true},
			{ spellKey = _W, range = 1000, speed = 1.6, delay = 300, width = 80, configName = "ArdentBlaze", displayName = "W (Ardent Blaze)", enabled = true, skillShot = true, minions = true, reset = false, reqTarget = false},
      }
      elseif myHero.charName == "Jinx" then
			spellArray = {
			{ spellKey = _W, range = 1450, speed = 3.3, delay = 600, width = 70, configName = "zap", displayName = "W (Zap!)", enabled = true, skillShot = true, minions = true, reset = false, reqTarget = false},
      }
		end
return spellArray
end
...
function mainMenu()
	...
	AutoCarry.MainMenu:addParam("MouseCarry", "Mouse Carry", SCRIPT_PARAM_ONKEYTOGGLE, false, MouseCarryKey)
...
function displayMenu()
	...
	DisplayMenu:addParam("MouseCarry", "Mouse Carry Hotkey Status", SCRIPT_PARAM_ONOFF, true)
...
function permaMenu()
	...
	if DisplayMenu.MouseCarry then AutoCarry.MainMenu:permaShow("MouseCarry") end
...
function masteryMenu()
	...
	MasteryMenu:addParam("Butcher", "Butcher 1=On/0=Off", SCRIPT_PARAM_SLICE, 0, 0, 1, 0)
	MasteryMenu:addParam("Spellblade", "Spellsword", SCRIPT_PARAM_ONOFF, false)
...
function farmMenu()
	...
	FarmMenu:addParam("MinionCircle", "Minion Marker Killable", SCRIPT_PARAM_ONOFF, true)
end
...
function OnWndMsg(msg,key)
    if msg == WM_LBUTTONDOWN and AutoCarry.MainMenu.MouseCarry == true then
            AutoCarry.MainMenu.AutoCarry = true
    elseif msg == WM_LBUTTONUP then
            AutoCarry.MainMenu.AutoCarry = false
    end
end